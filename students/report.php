...
$resultInfo = $conn->query("select * from results where test_id = '$test_id' AND student_id = '$stid'");

if(mysqli_num_rows($resultInfo) > 0){

    $conn->query("UPDATE results set obtained_marks = '$student_marks'");

}else {
    $conn->query("insert into results (test_id,student_id,obtained_marks,total_test_marks) VALUES('$test_id','$stid','$student_marks','$totalTestMarks')");
}
